...
#[derive(
	Clone, Eq, PartialEq, PartialOrd, Debug,
)]
...
impl<AccountId: std::cmp::PartialOrd + std::cmp::Eq, Balance: std::cmp::PartialOrd + std::cmp::Eq, BlockNumber: std::cmp::Eq + std::cmp::PartialOrd> Ord for Bid<AccountId, Balance, BlockNumber> {
    fn cmp(&self, other: &Self) -> Ordering {
        self.discount.cmp(&other.discount)
        ...
        egui::SidePanel::left("side_panel").show(ctx, |ui| {
            ...
            ui.group(|ui| {
                ...
                if ui.button("Add a Bid").clicked() {
                    let new_bid = create_bid(self.new_bid_amount, self.new_bid_discount, self.new_bid_index);
                    self.new_bid_index += 1;
                    self.bids.insert(new_bid);
                    self.log_messages
                        .push(format!("Generated a new bid with amount: {}", self.new_bid_amount));
                }
            });
            ...
            ui.label(format!("Account Liquidated: {}", self.liquidation.account_liquidated));
                ...
                Plot::new("bids_by_discount")
                    ...
                    .show(ui, |plot_ui| {
                        ...
                        for discount in 1..=20 {
                            ...
                            let bar = Bar::new(discount as f64, total_amount as f64)
                                .width(0.6); // adjust for a nicer spacing
                            bars.push(bar);
                        }

                        let chart = BarChart::new(bars)
                            .name("Total Bids by Discount");
                    ...
                    Plot::new("discount_emptied_plot")
                        ...
                        .allow_zoom(false)
                        .show(ui, |plot_ui| {
                            let chart = BarChart::new(bars_emptied)
                                .name("Empties by Discount");
                            plot_ui.bar_chart(chart);
                        });
...
fn liquidate(bids: &mut BTreeSet<UserBid>, liquidation: &mut SystemLiquidation) {
    ...
    while remaining_amount > 0 {
        ...
        println!("*** bid amount: {:?}, remaining amount: {:?}", current_bid.amount, remaining_amount);
